...
namespace Substrate
{
    ...
    public class ChunkRef : IChunk
    {
		...
		/// <summary>
		/// Fills this chunk with blocks of the given id from the given yLow altitude up to the given yHigh altitude. While this operation is running, every other operation on this ChunkRef is unsafe.
		/// </summary>
		/// <param name="blockId">The id of the blocks to fill the chunk with.</param>
		/// <param name="yLow">The low y coordinate to start filling at.</param>
		/// <param name="yHigh">The high y coordinate to stop filling at (yHigh is filled, as well).</param>
		public async Task FillAsync(int blockId, int yLow, int yHigh)
		{
			await Task.Run(
				() =>
				{
					Fill(blockId, yLow, yHigh);
				}
			);
		}
